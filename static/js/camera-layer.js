...
(function () {
  ...
  function initCamera() {
    ...
    const markers = cameras.map((cam) => {
      // bearing (0=北) に合わせて回転させる
      const rotation = cam.bearing || 0;

      // CSSで二等辺三角形（黒矢印）を作成
      // border-bottom:黒色部分が「上向き」の三角形になる
      // それを transform: rotate(...) で回転させる
      const arrowHtml = `
        <div style="
          width: 0; 
          height: 0; 
          ...
          opacity: 0.9;
          filter: drop-shadow(0px 0px 2px rgba(255,255,255,1)); /* 白縁取りで見やすく */
        "></div>
      `;
